...
export default function TypingTest() {
  ...
  // ─── FIX: always-fresh ref to handleTestComplete ───
  // The interval callback below is created inside an effect that only
  // depends on `testState`, so without this ref it would keep calling
  // whatever `handleTestComplete` closure existed at test-start time —
  // meaning auto-submit always scored against an empty userInput.
  ...
  useEffect(() => {
    if (testState === 'running') {
      timerRef.current = setInterval(() => {
        setTimeLeft((prev) => {
          if (prev <= 1) {
            handleTestCompleteRef.current(true); // ← always the latest version
            return 0;
          }
          return prev - 1;
        });
